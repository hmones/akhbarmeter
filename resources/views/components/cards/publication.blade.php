...
<x-card :title="$publication->title" :time="$publication->created_at->diffForHumans()" :tags="$publication->tags"
    ...
    <x-slot:image>
        <div
            class="w-full h-[266px] bg-cover bg-center bg-no-repeat"
            style="background-image: url('{{$publication->image ? Storage::url($publication->image) : asset('images/placeholders/article.png')}}')"
        >
        </div>
    </x-slot:image>
    <x-slot:description>
    ...
    <x-slot:extra>
        <div class="flex flex-row space-x-10 justify-between items-center rtl:space-x-reverse">
                ...
                <div class="flex flex-col">
                    ...
                    @else
                        <img src="https://www.gravatar.com/avatar/205e460b479e2e5b48aec07710c08d52?d=mp"
                             alt="{{$publication->author_name}} avatar" class="rounded-full w-12"/>
                    @endif
                </div>
                <div class="flex flex-col">
