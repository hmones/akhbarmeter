<div class="bg-blue-700 text-white py-20">
    <div class="container flex flex-col w-full items-center justify-center mx-auto space-y-4">
        <div class="flex flex-col justify-content-center text-center">
            <div class="text-4xl leading-10 font-extrabold tracking-tight">{{translate('components.support.header')}}</div>
        </div>
        <div class="flex-initial w-2/5 flex-col">
            <p class="text-center leading-6 font-normal">
                {{translate('components.support.description')}}
            </p>
        </div>
        <div class="flex-col">
            <button type="submit" class="mt-2 py-2.5 px-3 rounded bg-white hover:bg-gray-200 text-blue-600">
                {{translate('components.support.button')}}
            </button>
        </div>
    </div>
</div>
