...
@section('extra')
    <div class="flex space-x-10 justify-between items-center">
        <div class="flex">
            @if($avatar === "http://localhost/storage/")
                <img src="https://www.gravatar.com/avatar/205e460b479e2e5b48aec07710c08d52?d=mp" alt="{{$author}} avatar" class="rounded-circle w-12"/>
            @else
                <img src="{{$avatar}}" alt="{{$author}} avatar" class="rounded-circle w-12" />
            @endif
        </div>
        <div class="flex container mx-0 items-left justify-between">
            <div>
                ...
                <div class="flex flex-row text-sm leading-5 font-normal text-gray-500">
                    {{$created->format("F d, Y")}} . {{$read}} min to read
                </div>
            </div>
